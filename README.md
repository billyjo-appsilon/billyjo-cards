...
- **v0.4.2** (2026-05-22) — PC 가격박스 세로 중앙 정렬 픽스 (row 레이아웃 + align-items:center)
- **v0.4.1** (2026-05-22) — PC 가격박스(.fix_price)를 평가표 하단으로 이동 + 카드 시각 강화 (평가→가격 자연 시선 흐름)
- **v0.4.0** (2026-05-22) — 하단 위젯 fallback 콘텐츠 자체 생성 (`.bb-inner` 없어도 [가격][장바구니][🎁 렌탈+사은품 신청][💬 상담신청] 노출)
- **v0.3.9** (2026-05-22) — 카테고리 텍스트 전용 스와이프 (pill 폐기) + 좌측 정렬 + 활성 굵게/파랑+밑줄 (룰북 #20 갱신)
- **v0.3.8** (2026-05-22) — 카테고리 spacing 축소 + 자동 스크롤 정렬(.on 가운데/없으면 좌측) + 약정/의무사용 기간 ⓘ 툴팁
- **v0.3.7** (2026-05-22) — 모바일 검색바 명시 숨김 + 카테고리 1행 가로스크롤(룰북 #20 갱신) + 빨강→파랑 통일 + `.g-d` 평가없음 배지 높이 통일

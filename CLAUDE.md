...
16. **상세 스펙 — 섹션 전체 기본 접힘 + 2단계 펼침 (v0.8.3 갱신)**: SLOT 6 "상세 스펙"은 **섹션 전체를 기본 접힘**으로 둔다(공간 절약 — 사용자 명시 요구). 4지표 등급은 이미 SLOT 2 게이지에 노출되므로 섹션을 접어도 정보 손실 없음.
    - **(A) 섹션 collapse**: `<div class="sec">` 대신 `<details class="sec spec-collapse">` + `<summary><span class="sec-t">상세 스펙</span><span class="spec-collapse-hint"><span class="hint-open">펼쳐보기</span><span class="hint-close">접기</span></span></summary>` + `<div class="spec-collapse-body"> ... </div>`. 접힘 시 제목 + `펼쳐보기 ▾` pill만, 펼치면 Step 1-3 노출. CSS는 `details.spec-collapse`/`.spec-collapse-hint`/`.spec-line` (scripts/template-base.html 표준 블록). JS 불필요(네이티브 details).
    - **(B) Step 펼침(2단계)**: 펼친 뒤 각 Step (1-3)은 Step 번호 · 제목 · **등급 배지** · **제품 고유 요약 1줄**(`<strong>` 키워드)만 노출, pill·`.spec-line` 상세는 `<details class="step-details">` 안에 숨기고 "자세히 보기"로 펼침.
    - **(C) `.step-sum`은 제품 고유 내용 필수 — generic filler 금지**: "표준 {지표} — 카테고리 기본 기능 만족", "{지표} — 표준 수준" 같은 **모든 제품 동일 문구 사용 금지**. 해당 제품의 실제 스펙(빌리조 상세페이지 + 제조사 공식/보도자료)에서 추출한 고유 사실로 작성. 데이터가 충분하면 `.step-details` 안에 `.spec-line`(키-값) 행으로 실제 스펙 노출. 데이터가 부족하면 그 지표는 간결한 중립 라인 + (가능 시) `평가 없음`.
    - **(D) 앵커 보존**: `<!-- step-N-start --> ... <!-- step-N-end -->` 유지(규칙 #27). 외부 `spec-collapse` details가 per-step `step-details` details를 감싸도 valid.
    - **도구**: 신규/생성 카드는 `scripts/daily-sync.js`(스크랩 스펙 → 고유 요약+spec-line) + `scripts/template-base.html`. 기존 카드 일괄 전환은 `scripts/collapse-detailspec.js`(collapse 래핑 + filler→고유 요약, idempotent). 첫 레퍼런스: `cards/24578.html`(코웨이 아이콘 V2 CHPI-7400N).
...
30. **빌리조 로고 표기 — 한글·영문 cross-fade 패턴 (2026-05-30 신설)**: 빌리조 브랜드 로고는 **한글 로고(`빌리조`)** + **영문 로고(`billyjo`)** 두 가지가 공식 자산. UI에 노출할 때 **둘 중 하나만 정적 표시하지 말고, 2초마다 cross-fade로 번갈아 표시**한다 (브랜드 아이덴티티 양쪽 자연 강조).
    ...
    **inject.js 적용 surface (v0.6.9 갱신 — 빌리조 본 사이트)**: 본 사이트 로고는 원본 rentalshop 자산(`rentalshop.site/_data/.../8237a1...png`)을 쓰지만, 우리는 그 위에 **admin2 호스팅 자산(`admin2.billyjo.co.kr/logo/billyjo-ko.png`·`billyjo-en.png`)으로 교체 + 2초 cross-fade**를 입힌다. CSS keyframes(admin2) 대신 inject.js는 **JS setInterval(2000ms) + opacity 토글** 방식 — 원본 img를 ko로 교체하고 en clone을 `position:absolute; width/height:100%; object-fit:contain`으로 같은 자리에 겹쳐 idempotent하게 토글:
    - **헤더 로고(`.logo > img`, `header .logo img`)** — 모듈 B(`alternateBillyjoLogo`, prod_view 전용)에서 적용. dataset `bjLogoAlt`·class `bj-logo-alt-ko/en`·`window.__bjLogoAltInterval` 가드.
...
33. **하단 위젯 단일 소유권 + fail-open 의무 (2026-06-07 신설)**: 상세페이지 하단 위젯(렌탈신청·약정 pill·상담신청)은 **모듈 B의 `.prod_view_bot.card.mt40` wrapper가 단일 소유**한다. 어떤 페이지에서도 위젯이 비어 있거나 사라진 채로 끝나면 안 된다.
    ...
    **(D) 터치 디바이스 검증 + 합성 마우스 가드 (2026-06-07 신설)**: 탭/드래그로 조작하는 인터랙티브 위젯은 **Playwright `hasTouch: true` 컨텍스트의 `touchscreen.tap()`** 으로 검증해야 한다 — `mouse.click()`·`dispatchEvent(MouseEvent)`는 터치 기기의 실제 이벤트 시퀀스를 재현하지 못한다.
    - **원인 패턴**: 터치 기기에서 `touchend` 후 브라우저가 합성(compatibility) `mousedown/mouseup`을 추가 발사. mouse·touch 양쪽에 같은 핸들러를 바인딩하면 **탭 1회 = 토글 2회** → "위젯이 나왔다가 바로 사라지는" 증상 (2026-06-07 아이콘 V2 정수기 모바일 신고 — 데스크톱 마우스만으로 테스트해 통과한 회귀).
    - **표준 가드** (`setupHandleDragGesture` 참조): touch 이벤트 수신 시 `lastTouchTs` 기록, mouse 이벤트는 `Date.now() - lastTouchTs < 800ms`면 무시. 마우스 전용 환경은 touch 이벤트가 없어 가드에 안 걸림.
    - 검증 시퀀스: 터치 탭 3회(펼침→접힘→펼침 토글 일관) + 마우스 클릭 1회(데스크톱 회귀) 모두 통과해야 배포.
...
- v0.8.2 (2026-06-07): **#33(D) 터치 검증 조항 신설 — 위젯 탭 이중 토글 fix** (billyjo-inject). 아이콘 V2 정수기 모바일 "위젯이 나왔다가 바로 사라짐" 신고 → 재현: 터치 기기에서 touchend 토글(펼침) 직후 합성 mousedown/mouseup이 재토글(접힘). setupHandleDragGesture에 lastTouchTs 800ms 합성 마우스 무시 가드. 터치 탭 3회 토글 + 마우스 회귀 검증 통과.
- v0.8.3 (2026-06-23): **상세 스펙(SLOT 6) 기본 접힘 + generic filler 제거 (절대 규칙 #16 갱신)**. 사용자 신고: "상세 스펙 내용이 너무 아무내용 없이 일관된 내용만 나와서" + "평소에는 접어둬서 공간을 아끼자". (1) SLOT 6 섹션 전체를 `<details class="sec spec-collapse">`로 감싸 **기본 접힘** — 접힘 시 `상세 스펙 + 펼쳐보기 ▾` pill만, 펼치면 Step 1-3. (2) 모든 제품 동일하게 나오던 generic filler step-sum 3종 변형("표준 {지표} — 카테고리 기본 기능 만족", "{지표} — 표준 수준 (상세 미공개...)", "{지표} — 표준 수준")을 **제품 고유 요약**으로 교체 — 데이터 있는 step은 실제 활성 pill/스펙에서 추출(`<strong>` 강조), 부족하면 간결한 중립 라인. (3) `.spec-line`(키-값) CSS 신설로 펼친 상세에 실제 스펙 행 노출 가능. **반영 범위**: 첫 레퍼런스 `cards/24578.html`(코웨이 아이콘 V2 CHPI-7400N — 제조사 보도자료 기반 나노트랩/4중 UV/100℃ 등 고유 사실) → `scripts/template-base.html` + `scripts/daily-sync.js` 생성기(신규 카드) + `scripts/collapse-detailspec.js` 일괄(기존 3,349개, idempotent, 수기 요약 카드 보존). 검증: 320/360/390/430px 가로 넘침 0(규칙 #32), details/anchor 균형 OK, 재실행 시 alreadyDone 3350. **남은 한계**: 제너릭 path로 생성된 비-F01 기존 카드는 step 데이터가 빈약(title-pill만) → 깊은 고유 내용은 개선된 생성기로 재스크랩하거나 제품별 리서치(24578 방식) 필요 — 후속 작업.
...
- v0.8.4 (2026-06-29): **하단 위젯 토글 UX 재설계 + 헤더 아이콘 옅은 회색 (절대 규칙 #25 갱신, billyjo-inject `0bb6158`)**. (1) **위젯 토글 단순화**: 별도 ▲ 토글 박스 버튼(`.bj-bar-handle-toggle`)을 제거(`display:none`) — 핸들 전체 탭으로 토글되므로 중복. 중앙 grip 셰브론(`.bj-bar-handle::before`)을 **단일 토글 인디케이터**로 승격: 접힘 시 위(^), 펼침 시 180° 회전해 아래(⌄)를 가리킴(`:not(.bj-bar-collapsed)` selector + transform transition). (2) **'옵션 선택' 칩 우측 정렬**(`.bj-bar-handle-text .bj-bar-handle-option { margin-left:auto }`) — 접힘 핸들은 `[월 렌탈료(좌) · grip(중앙) · 옵션 선택(우)]`. (3) **헤더 검색/장바구니 아이콘 색**: 브랜드 블루(#0838f8)가 너무 튄다는 피드백 → 상단 메뉴바 구분선(`#eee`) 톤의 옅은 회색 **#aaaaaa**로(아이콘은 #eee면 안 보여 가독성 있는 #aaa 채택). 사용자 피드백: "쉐브론 클릭하면 펼쳐지고 펼침 시 아래 향하게 + 펼침/접힘 버튼 없애고 + 옵션 선택은 우측 정렬" / "돋보기·장바구니 파란색이 튀니 구분선 같은 옅은 회색으로". **위치 결정**: 위젯 라이브 코드는 billyjo-inject(모듈 B, 통합 정책) 소관 — cards 레포에는 이 UX 규칙만 기록(이 항목 + DESIGN.md 하단 sticky 구매 바 bullet).

...
20. **빌리조 카테고리 메뉴 (`.category__wrap`) 1행 가로 스크롤 (v0.3.7 갱신)**: 빌리조 본 사이트 `.mobile__gnb .gnb__cateogry .category__wrap` (제품 상세 페이지 하단 가까이 노출되는 모바일 카테고리 메뉴). v0.2.3에서 "2줄 제한 + ▼ 토글" 패턴이었으나 토글 없이 한눈에 모든 카테고리를 빠르게 스캔할 수 있게 **1행 가로 스크롤** 패턴으로 변경:
    - `.category__wrap { display:flex; flex-wrap:nowrap; overflow-x:auto; -webkit-overflow-scrolling:touch; gap:6px; padding:8px 12px; white-space:nowrap; scrollbar-width:none }` + `::-webkit-scrollbar { display:none }` — 스크롤바는 숨김(시각 깔끔)
    - 각 카테고리는 pill 형태 — `flex:0 0 auto; padding:6px 12px; border:0.5px solid #e5e5e5; border-radius:999px; background:#f4f4f4; color:#555; font-size:12.5px; font-weight:600; white-space:nowrap; line-height:1.3`
    - 활성(`.on` 또는 `:active`): 브랜드 파랑 채움 `background:#0838F8; color:#fff; font-weight:700`
    - 호버: 연한 파란 배경 `#e8edff; color:#0838F8; border:#0838F8`
    - `.gnb__cateogry::after` 우측 fade 그라데이션 — 스크롤 가능 신호 (`linear-gradient(to right, transparent, #fff)`, width 24px)
    - 원본 `.on::after` 주황 밑줄은 `display:none` 처리
    - **이전 v0.2.3의 ▼ 토글 JS는 폐기** — 토글 자체가 사용자에게 추가 인지 부담. 가로 스크롤이 카테고리 다 보기 더 직관적.
    - 모든 패치는 CSS only (JS 필요 없음).
